import 'package:fitness_app/config/base_response/base_response.dart';
import 'package:fitness_app/core/request/query_request.dart';
import 'package:fitness_app/feature/food/data/models/meal/meal_details_response.dart';
import 'package:fitness_app/feature/food/data/repo/meal_repo_impl.dart';
import 'package:fitness_app/feature/food/domain/models/meals_entity.dart';
import 'package:fitness_app/feature/food/domain/use_cases/get_meal_details_use_case.dart';
import 'package:fitness_app/feature/food/domain/use_cases/get_meals_use_case.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/annotations.dart';
import 'package:mockito/mockito.dart';
import 'get_meals_use_case_test.mocks.dart';
@GenerateMocks([MealRepoImpl])
void main() {
  test('Test for get meal details use case ', () {
    final MealRepoImpl repoImpl=MockMealRepoImpl();
    final GetMealDetailsUseCase useCase=GetMealDetailsUseCase(repoImpl);
  final MealDetailsResponse mealDetailsResponse=MealDetailsResponse(
        meals: [
          MealDto(
              strCategory: 'seafood'
          )
        ]
    );
    const DynamicQueries request= DynamicQueries(queriesData:
    [QueryData(key: 'c',value: 'seafood')]
    );
    provideDummy<BaseResponse<MealDetailsResponse>>(Success(mealDetailsResponse));
    when(repoImpl.getMealDetails(request)).thenAnswer((_) async => Success(mealDetailsResponse));
    useCase.invoke(request);
    verify(repoImpl.getMealDetails(request));
  },);

}